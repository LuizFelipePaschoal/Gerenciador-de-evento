...
public class SubscriptionService {
    ...
    public SubscriptionResponse createNewSubscription(String eventName, User user, Integer userId) {
        ...
        if (tmpSub != null) {
            throw new SubscriptionConflictException(
                    "ja existe inscrição para o usuário " + userRec.getName() + " no evento " + evt.getTitle());
        }

        Subscription res = subRepo.save(subs);
        return new SubscriptionResponse(res.getSubscripionNumber(), "http://codecraft.com/subscription/"
    ...
    public SubscriptionRankingByUser getRankingByUser(String prettyName, Integer userId) {
        ...
        if (item == null) {
            throw new UserIndicadorNotFoundException("Não há inscriçoes com indicacao do usuario "+userId);
        }
